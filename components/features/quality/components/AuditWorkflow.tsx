...
export const AuditWorkflow: React.FC<AuditWorkflowProps> = ({ 
  ...
  return (
    <div className="space-y-10 pb-24" role="list" aria-label={t('audit.workflow.title')}>
        
        {/* 1. LIBERAÇÃO VITAL (SGQ) */}
        <StepCard 
          step={1} title={t('audit.workflow.steps.step1')}
        ...
        >
          {isQuality && !s1 && (
            ...
            >
              {isSyncing ? <Activity className="animate-spin" size={18}/> : t('audit.workflow.labels.authFlow')}
            </button>
          )}
        </StepCard>
        ...
        <StepCard 
          step={2} title={t('audit.workflow.steps.step2')} 
        ...
        >
            <div className="space-y-6">
                ...
                >
                    {s2 ? <><Eye size={18} /> {t('audit.workflow.labels.viewNotes')}</> : <><FileText size={18} /> {t('audit.workflow.labels.stationAnnotation')}</>}
                </button>

                {((isClient && s1 && !s2) || (s2)) && (
                    <div className="space-y-6 p-6 bg-slate-50 rounded-2xl border-2 border-slate-200 shadow-inner">
                        <div className="space-y-3">
                            <label className="text-xs font-black text-slate-600 uppercase tracking-widest ml-1">{t('audit.workflow.labels.attachDocs')}</label>
                            <p className="text-[10px] text-slate-400 font-medium leading-relaxed mb-3">
                                {t('audit.workflow.labels.attachDocsDesc')}
                            </p>
                            <EvidenceUploadArea 
                              step="documental" 
                        ...
                        <div className="space-y-3">
                            <label className="text-xs font-black text-slate-600 uppercase tracking-widest ml-1">{t('audit.workflow.labels.auditFlags')}</label>
                            <div className="flex flex-wrap gap-2">
                                ...
                                {isClient && !s2 && (
                                    <div className="flex gap-2 w-full mt-2">
                                        <input 
                                            type="text" 
                                            placeholder={t('audit.workflow.labels.addFlag')} 
                        ...
                        <div className="space-y-3">
                            <label className="text-xs font-black text-slate-600 uppercase tracking-widest ml-1">{t('audit.workflow.labels.reportDivergence')}</label>
                            <textarea 
                                ...
                                placeholder={t('audit.workflow.labels.reportDivergencePlaceholder')}
                            />
                        </div>
                    </div>
                )}
                ...
                {isClient && s1 && !s2 && (
                    ...
                    >
                        <Check size={18} strokeWidth={3} /> {t('audit.workflow.labels.signValidation')}
                    </button>
                )}
            </div>
        </StepCard>
        ...
        {/* 3. VISTORIA DE CARGA */}
        <StepCard 
          step={3} title={t('audit.workflow.steps.step3')} 
            ...
            <div className="space-y-6">
                {((isClient && s1 && !s3) || (s3)) && (
                    <div className="space-y-6 p-6 bg-slate-50 rounded-2xl border-2 border-slate-200 shadow-inner">
                        ...
                        <div className="space-y-3">
                            <label className="text-xs font-black text-slate-600 uppercase tracking-widest ml-1">{t('audit.workflow.labels.galleryField')}</label>
                            <p className="text-[10px] text-slate-400 font-medium leading-relaxed mb-3">
                                {t('audit.workflow.labels.galleryFieldDesc')}
                            </p>
                            <EvidenceUploadArea 
                                step="physical" 
                        ...
                        <div className="space-y-3">
                            <label className="text-xs font-black text-slate-600 uppercase tracking-widest ml-1">{t('audit.workflow.labels.physicalState')}</label>
                            <div className="flex flex-wrap gap-2">
                        ...
                        <div className="space-y-3">
                            <label className="text-xs font-black text-slate-600 uppercase tracking-widest ml-1">{t('audit.workflow.labels.physicalNotes')}</label>
                            <textarea 
                                readOnly={s3 || !isClient}
                                value={s3 ? (metadata?.physicalNotes || '') : physNotes}
                                onChange={e => setPhysNotes(e.target.value)}
                                className="w-full p-4 bg-white border-2 border-slate-200 rounded-xl text-sm min-h-[100px] outline-none focus:ring-4 focus:ring-blue-500/10 font-medium leading-relaxed"
                                placeholder={t('audit.workflow.labels.physicalNotesPlaceholder')}
                            />
                        </div>
                    </div>
                )}
                ...
                {isClient && s1 && !s3 && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        >
                            <Check size={18} strokeWidth={3} /> {t('audit.workflow.labels.approveLoad')}
                        </button>
                        <button 
                        ...
                        >
                            <XCircle size={18} /> {t('audit.workflow.labels.rejectLoad')}
                        </button>
                    </div>
                )}
            </div>
        ...
        <StepCard 
          step={4} title={t('audit.workflow.steps.step4')} 
            ...
            <div className="space-y-6">
                {isStep4AutoCompleted && !s4 && (
                    <div className="p-5 bg-emerald-50 rounded-2xl border-2 border-emerald-200 flex items-center gap-4 shadow-sm" role="status">
                        ...
                        <p className="text-xs font-black text-emerald-900 uppercase tracking-tight leading-relaxed">{t('audit.workflow.labels.fullCompliance')}: <br/><span className="font-medium normal-case text-emerald-700 opacity-80">{t('audit.workflow.labels.noDivergence')}</span></p>
                    </div>
                )}
                {isQuality && isArbitrationNeeded && !s4 && (
                    <div className="space-y-4">
                        <textarea 
                            ...
                            placeholder={t('audit.workflow.labels.arbitrationPlaceholder')}
                        ...
                        >
                            {isSyncing ? <Activity className="animate-spin" size={18}/> : <><Lock size={18} /> {t('audit.workflow.labels.signArbitration')}</>}
                        </button>
                    </div>
                )}
                {s4 && metadata?.arbitrationNotes && (
                    <div className="p-6 bg-blue-50 rounded-2xl border-2 border-blue-100 space-y-3 shadow-inner">
                        <p className="text-xs font-black text-blue-500 uppercase tracking-widest flex items-center gap-2">
                          <MessageSquare size={16} /> {t('audit.workflow.labels.vitalMediationNote')}:
                        </p>
        ...
        <StepCard 
          step={5} title={t('audit.workflow.steps.step5')} 
        ...
        >
            <div className="space-y-6">
                {isClient && isStep4Done && !s5 && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <button onClick={() => handleAction('step5_partner_verdict', { status: QualityStatus.APPROVED })} className="py-4 bg-emerald-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl hover:bg-emerald-700 transition-all active:scale-95">{t('audit.workflow.labels.acceptBatch')}</button>
                        <button onClick={() => handleAction('step5_partner_verdict', { status: QualityStatus.REJECTED })} className="py-4 bg-red-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl hover:bg-red-700 transition-all active:scale-95">{t('audit.workflow.labels.rejectBatch')}</button>
                    </div>
                )}
                {s5 && (
                    <div className={`p-6 rounded-2xl border-2 flex items-center gap-5 shadow-sm ${metadata?.status === QualityStatus.APPROVED ? 'bg-emerald-50 border-emerald-200 text-emerald-800' : 'bg-red-50 border-red-200 text-red-800'}`}>
                        ...
                        <div>
                            <p className="text-xs font-black uppercase tracking-tight opacity-60">{t('audit.workflow.labels.clientAcceptance')}:</p>
                            <p className="text-lg font-black uppercase tracking-widest">{metadata?.status === QualityStatus.APPROVED ? t('audit.workflow.labels.homologated') : t('audit.workflow.labels.refused')}</p>
                        </div>
                    </div>
                )}
            </div>
        ...
        <StepCard step={6} title={t('audit.workflow.steps.step6')} completed={s6} active={s5 && !s6} icon={Lock}>
            <div className="space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <StatusSlot label={t('audit.workflow.labels.repPartner')} signed={s6_c} signature={sigs.step6_consolidation_client} />
                    <StatusSlot label={t('audit.workflow.labels.repQuality')} signed={s6_q} signature={sigs.step6_consolidation_quality} />
                </div>
                {s5 && !s6 && (
                    <div className="pt-2">
                        {isClient && !s6_c && <button onClick={() => handleAction('step6_consolidation_client', {})} className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl hover:bg-black transition-all">{t('audit.workflow.labels.signDigitalSealClient')}</button>}
                        {isQuality && !s6_q && <button onClick={() => handleAction('step6_consolidation_quality', {})} className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl hover:bg-black transition-all">{t('audit.workflow.labels.signDigitalSealAnalyst')}</button>}
                    </div>
                )}
            </div>
        </StepCard>
        ...
        <StepCard step={7} title={t('audit.workflow.steps.step7')} completed={s6} active={s6} icon={Award}>
            {s6 && (
                    ...
                    {!isRejected ? (
                        <div className="flex items-center gap-6 p-8 bg-emerald-600 text-white rounded-[2rem] shadow-2xl relative overflow-hidden group">
                            ...
                            <div className="relative z-10">
                                <h4 className="text-lg font-black uppercase tracking-tight leading-none">{t('audit.workflow.labels.vitalCertification')}</h4>
                                <p className="text-xs font-bold text-white/80 mt-1.5 uppercase tracking-widest">{t('audit.workflow.labels.homologatedAsset')}</p>
                            </div>
                        </div>
                    ) : (
                        <div className="space-y-6">
                            <div className="flex items-center gap-5 p-6 bg-red-50 rounded-2xl border-2 border-red-200">
                                ...
                                <div className="flex-1">
                                    <h4 className="text-sm font-black text-red-900 uppercase tracking-tight">{t('audit.workflow.labels.transmissionBlocked')}</h4>
                                    <p className="text-xs text-red-700 font-medium leading-relaxed mt-1">
                                        {isQuality 
                                            ? t('audit.workflow.labels.rejectedByPartner')
                                            : t('audit.workflow.labels.criticalDivergence')}
                                    </p>
                                </div>
                            </div>
                            ...
                            {isQuality && clientRep && (
                                <div className="p-6 bg-white border-2 border-slate-200 rounded-[2rem] space-y-4 shadow-sm">
                                    <header className="flex items-center gap-2 text-slate-50">
                                        <User size={16} />
                                        <span className="text-[11px] font-black uppercase tracking-widest">{t('audit.workflow.labels.partnerContact')}</span>
                                    </header>
            ...
            <div className="flex flex-wrap gap-3">
                ...
                {photos.length === 0 && disabled && (
                    <div className="w-full py-4 border-2 border-dashed border-slate-200 rounded-xl flex flex-col items-center justify-center text-slate-300 grayscale opacity-50">
                        <ImageIcon size={24} />
                        <span className="text-[8px] font-black uppercase mt-1">{t('audit.workflow.messages.noImages')}</span>
                    </div>
                )}
            </div>
        </div>
        ...
        <div className={`p-5 rounded-2xl border-2 transition-all ${signed ? 'bg-emerald-50 border-emerald-200 shadow-sm' : 'bg-slate-50 border-slate-200 opacity-60'}`}>
            ...
            {signed ? (
                <div className="flex items-center gap-2 text-emerald-700">
                    ...
                    <span className="text-xs font-black uppercase tracking-tight">{t('audit.workflow.labels.signed')}</span>
                </div>
            ) : (
                <div className="flex items-center gap-2 text-slate-400">
                    <Clock size={16} />
                    <span className="text-xs font-bold uppercase tracking-tight">{t('audit.workflow.labels.waiting')}</span>
                </div>
            )}
            {signature && (
                        ...
                        <div className="space-y-1">
                            ...
                            <div className="flex items-center gap-2">
                              <span className={`text-[10px] font-black px-2.5 py-1 rounded-lg uppercase tracking-widest shadow-sm ${
                                ...
                                {completed ? t('audit.workflow.labels.protocolValidated') : active ? t('audit.workflow.labels.phaseAnalysis') : t('audit.workflow.labels.awaitingRelease')}
                              </span>
                            </div>
                        </div>
                    ...
                    {signature && (
                        <div className="mt-6 p-6 bg-slate-50 rounded-[2rem] border-2 border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-6 shadow-inner">
                            <div className="flex-1 min-w-0">
                                <p className="text-[10px] font-black text-slate-500 uppercase tracking-[3px] mb-2">{t('audit.workflow.labels.digitalSignature')}:</p>
                            ...
                            <div className="text-left sm:text-right shrink-0 bg-white/60 px-4 py-2 rounded-2xl border border-slate-200/50">
                                <p className="text-[11px] font-black text-slate-700 font-mono">{new Date(signature.timestamp).toLocaleDateString()}</p>
                                <p className="text-[11px] font-black text-slate-400 font-mono mt-0.5">{new Date(signature.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                            </div>
                        </div>
                    )}
                </div>
